...
public class productsUpdateForm extends JFrame {
	// GRID FORMATTING
	private final int GRIDWIDTH 	= 900;		// size our grid 
	private final int GRIDHEIGHT	= 300;


	productsUpdatePanelBuilder userInfoPanel;		// panel for user information
	...
	public productsUpdateForm() throws SQLException {
		...
		userInfoPanel = new productsUpdatePanelBuilder();
	...
	private void buildListPanel() throws SQLException {
		...
		productsTableManager um = new productsTableManager();
				...
				public void valueChanged(ListSelectionEvent event) {
					...
					if (ourTable.getSelectedRow() >= 0) {
						...
						try {
							...
							ResultSet updateInfo = productsTableManager.selectUpdate(selectedRecordID);

							while(updateInfo.next()) {
								userInfoPanel.setProduct(updateInfo.getString("Product"));
								userInfoPanel.setCat(updateInfo.getInt("Category"));
	...
	private void resetForm() throws SQLException {
		...
		searchInfo = productsTableManager.selectUsers("", "");
	...
	// Handle search, filter button
		public class SearchButtonListener implements ActionListener {
			...
			public void actionPerformed(ActionEvent e1) {
				...
				String strProduct = userInfoPanel.getProduct();
				String cat = userInfoPanel.getCat();
				...
				try {
					...
					productsTableManager getInfo = new productsTableManager();
					// create a blank search of the table
					searchInfo = productsTableManager.selectUsers(strProduct, cat);
	...
	public class InsertButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (cat.equals("")){
				JOptionPane.showMessageDialog(null, "Cannot insert a record with no category.");
				return;
			}
			...
			try {
				...
				productsTableManager userManager = new productsTableManager();
				
				// insert the record
				rows = userManager.insertRecord(strProduct, cat);
	...
	public class UpdateButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e1) {
			...
			int rows = 0;
			productsTableManager updater;
			
			
			try {
				// create an object to instantiate the Connection to the table	
				updater = new productsTableManager();
				rows = updater.updateRecord(strProduct, cat, selectedRecordID);
	...
	public class DeleteButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e1) {
			...
			int rows = 0;
			productsTableManager Deleter;
			
			
			try {
				// create an object to instantiate the Connection to the table	
				Deleter = new productsTableManager();
	...
	public static void main(String[] arg0) throws SQLException {
		productsUpdateForm dbgs = new productsUpdateForm();
	}
}

...
public class ordersUpdateForm extends JFrame {
	// GRID FORMATTING
	private final int GRIDWIDTH 	= 900;		// size our grid 
	private final int GRIDHEIGHT	= 300;


	ordersUpdatePanelBuilder userInfoPanel;		// panel for user information
	...
	public ordersUpdateForm() throws SQLException {
		...
		userInfoPanel = new ordersUpdatePanelBuilder();
	...
	private void buildListPanel() throws SQLException {
		...
		ordersTableManager um = new ordersTableManager();
				...
				public void valueChanged(ListSelectionEvent event) {
					...
					if (ourTable.getSelectedRow() >= 0) {
						...
						try {
							...
							ResultSet updateInfo = ordersTableManager.selectUpdate(selectedRecordID);

							while(updateInfo.next()) {
								userInfoPanel.setTime(updateInfo.getString("Time"));
								userInfoPanel.setAmount(updateInfo.getString("Amount"));
								userInfoPanel.setProduct(updateInfo.getInt("Product"));
	...
	private void resetForm() throws SQLException {
		...
		searchInfo = ordersTableManager.selectUsers("", "", "");
	...
	// Handle search, filter button
		public class SearchButtonListener implements ActionListener {
			...
			public void actionPerformed(ActionEvent e1) {
				...
				String strAmount = userInfoPanel.getAmount();
				String prod = userInfoPanel.getProduct();
				...
				try {
					...
					ordersTableManager getInfo = new ordersTableManager();
					// create a blank search of the table
					searchInfo = ordersTableManager.selectUsers(strTime, strAmount, prod);
	...
	public class InsertButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (strTime.equals("")){
				JOptionPane.showMessageDialog(null, "Cannot insert a record with no time.");
				return;
			}
			
			if (strAmount.equals("")){
				JOptionPane.showMessageDialog(null, "Cannot insert a record with no amount.");
				return;
			}
			
			if (prod.equals("")){
				JOptionPane.showMessageDialog(null, "Cannot insert a record with no product.");
				return;
			}
			...
			try {
				...
				ordersTableManager userManager = new ordersTableManager();
				
				// insert the record
				rows = userManager.insertRecord(strTime, strAmount, prod);
	...
	public class UpdateButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e1) {
			...
			int rows = 0;
			ordersTableManager updater;
			
			
			try {
				// create an object to instantiate the Connection to the table	
				updater = new ordersTableManager();
				rows = updater.updateRecord(strTime, strAmount, prod, selectedRecordID);
	...
	public class DeleteButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e1) {
			...
			int rows = 0;
			ordersTableManager Deleter;
			
			
			try {
				// create an object to instantiate the Connection to the table	
				Deleter = new ordersTableManager();
	...
	public static void main(String[] arg0) throws SQLException {
		ordersUpdateForm dbgs = new ordersUpdateForm();
	}
}

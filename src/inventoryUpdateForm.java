...
public class inventoryUpdateForm extends JFrame {
	// GRID FORMATTING
	private final int GRIDWIDTH 	= 900;		// size our grid 
	private final int GRIDHEIGHT	= 300;


	inventoryUpdatePanelBuilder userInfoPanel;		// panel for user information
	...
	public inventoryUpdateForm() throws SQLException {
		...
		userInfoPanel = new inventoryUpdatePanelBuilder();
	...
	private void buildListPanel() throws SQLException {
		...
		listPanel.setBorder(BorderFactory.createTitledBorder("Inventory Information"));
		
		// create an object to access the db
		inventoryTableManager um = new inventoryTableManager();
				...
				public void valueChanged(ListSelectionEvent event) {
					...
					if (ourTable.getSelectedRow() >= 0) {
						...
						try {
							...
							ResultSet updateInfo = inventoryTableManager.selectUpdate(selectedRecordID);

							while(updateInfo.next()) {
								userInfoPanel.setItem(updateInfo.getString("Count"));
								userInfoPanel.setCombo(updateInfo.getInt("Item"));
							}
	...
	private void resetForm() throws SQLException {
		...
		searchInfo = inventoryTableManager.selectUsers("", "");
		...
		public class SearchButtonListener implements ActionListener {
			...
			public void actionPerformed(ActionEvent e1) {
				...
				try {
					...
					inventoryTableManager getInfo = new inventoryTableManager();
					// create a blank search of the table
					searchInfo = inventoryTableManager.selectUsers(strCombo, strItem);
	...
	public class InsertButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			try {
				...
				inventoryTableManager userManager = new inventoryTableManager();
				
				// insert the record
				rows = userManager.insertRecord(strItem, strCombo);
	...
	public class UpdateButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e1) {
			...
			int rows = 0;
			inventoryTableManager updater;
			
			
			try {
				// create an object to instantiate the Connection to the table	
				updater = new inventoryTableManager();
				rows = updater.updateRecord(strItem, strCombo, selectedRecordID);
	...
	public class DeleteButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e1) {
			...
			int rows = 0;
			inventoryTableManager Deleter;
			
			
			try {
				// create an object to instantiate the Connection to the table	
				Deleter = new inventoryTableManager();
	...
	public static void main(String[] arg0) throws SQLException {
		inventoryUpdateForm dbgs = new inventoryUpdateForm();
	}
}

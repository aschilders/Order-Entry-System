...
public class menuItemUpdateForm extends JFrame {
	// GRID FORMATTING
	private final int GRIDWIDTH 	= 900;		// size our grid 
	private final int GRIDHEIGHT	= 300;


	menuItemUpdatePanelBuilder userInfoPanel;		// panel for user information
	...
	public menuItemUpdateForm() throws SQLException {
		...
		userInfoPanel = new menuItemUpdatePanelBuilder();
	...
	private void buildListPanel() throws SQLException {
		...
		listPanel.setBorder(BorderFactory.createTitledBorder("Menu Item Information"));
		
		// create an object to access the db
		menuItemTableManager um = new menuItemTableManager();
				...
				public void valueChanged(ListSelectionEvent event) {
					...
					if (ourTable.getSelectedRow() >= 0) {
						...
						try {
							...
							ResultSet updateInfo = menuItemTableManager.selectUpdate(selectedRecordID);

							while(updateInfo.next()) {
								userInfoPanel.setItem(updateInfo.getString("Itemname"));
								userInfoPanel.setPrice(updateInfo.getString("Price"));
								userInfoPanel.setDesc(updateInfo.getString("Description"));
								userInfoPanel.setCombo(updateInfo.getInt("Type"));
							}
	...
	private void resetForm() throws SQLException {
		...
		searchInfo = menuItemTableManager.selectUsers("", "", "", "");
	...
	// Handle search, filter button
		public class SearchButtonListener implements ActionListener {
			...
			public void actionPerformed(ActionEvent e1) {
				...
				String strItem = userInfoPanel.getItem();
				String strPrice = userInfoPanel.getPrice();
				String strDesc = userInfoPanel.getDesc();
				...
				try {
					...
					menuItemTableManager getInfo = new menuItemTableManager();
					// create a blank search of the table
					searchInfo = menuItemTableManager.selectUsers(strItem, strCombo, strPrice, strDesc);
	...
	public class InsertButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			try {
				...
				menuItemTableManager userManager = new menuItemTableManager();
				
				// insert the record
				rows = userManager.insertRecord(strItem, strPrice, strDesc, strCombo);
	...
	public class UpdateButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e1) {
			...
			int rows = 0;
			menuItemTableManager updater;
			
			
			try {
				// create an object to instantiate the Connection to the table	
				updater = new menuItemTableManager();
				rows = updater.updateRecord(strItem, strPrice, strDesc, strCombo, selectedRecordID);
	...
	public class DeleteButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e1) {
			...
			int rows = 0;
			menuItemTableManager Deleter;
			
			
			try {
				// create an object to instantiate the Connection to the table	
				Deleter = new menuItemTableManager();
	...
	public static void main(String[] arg0) throws SQLException {
		menuItemUpdateForm dbgs = new menuItemUpdateForm();
	}
}

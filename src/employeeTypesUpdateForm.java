...
public class employeeTypesUpdateForm extends JFrame {
	// GRID FORMATTING
	private final int GRIDWIDTH 	= 900;		// size our grid 
	private final int GRIDHEIGHT	= 300;


	employeeTypesUpdatePanelBuilder userInfoPanel;		// panel for user information
	...
	public employeeTypesUpdateForm() throws SQLException {
		// set the window title
		setTitle("Update Employee Types");
		// set our action when the user close
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		
		// create a panel builder object
		userInfoPanel = new employeeTypesUpdatePanelBuilder();
	...
	private void buildListPanel() throws SQLException {
		...
		listPanel.setBorder(BorderFactory.createTitledBorder("Employee Type Information"));
		
		// create an object to access the db
		employeeTypesTableManager um = new employeeTypesTableManager();
				...
				public void valueChanged(ListSelectionEvent event) {
					...
					if (ourTable.getSelectedRow() >= 0) {
						...
						try {
							...
							ResultSet updateInfo = employeeTypesTableManager.selectUpdate(selectedRecordID);

							while(updateInfo.next()) {
								userInfoPanel.setEmployeeType(updateInfo.getString("Permission"));
								userInfoPanel.setEmployeeDesc(updateInfo.getString("Description"));
							}
	...
	private void resetForm() throws SQLException {
		...
		searchInfo = employeeTypesTableManager.selectUsers("", "");
	...
	// Handle search, filter button
		public class SearchButtonListener implements ActionListener {
			...
			public void actionPerformed(ActionEvent e1) {
				...
				String strType = userInfoPanel.getEmployeeType();
				String strDesc = userInfoPanel.getEmployeeDesc();
				...
				try {
					...
					employeeTypesTableManager getInfo = new employeeTypesTableManager();
					// create a blank search of the table
					searchInfo = employeeTypesTableManager.selectUsers(strType, strDesc);
	...
	public class InsertButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (strType.equals("")){
				JOptionPane.showMessageDialog(null, "Cannot insert a record with no type.");
				return;
			}
			
			if (strDesc.equals("")){
				JOptionPane.showMessageDialog(null, "Cannot insert a record with no description.");
				return;
			}
			...
			try {
				...
				employeeTypesTableManager userManager = new employeeTypesTableManager();
				
				// insert the record
				rows = userManager.insertRecord(strType, strDesc);
	...
	public class UpdateButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e1) {
			...
			int rows = 0;
			employeeTypesTableManager updater;
			
			
			try {
				// create an object to instantiate the Connection to the table	
				updater = new employeeTypesTableManager();
				rows = updater.updateRecord(strType, strDesc, selectedRecordID);
	...
	public class DeleteButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e1) {
			...
			int rows = 0;
			employeeTypesTableManager Deleter;
			
			
			try {
				// create an object to instantiate the Connection to the table	
				Deleter = new employeeTypesTableManager();
	...
	public static void main(String[] arg0) throws SQLException {
		employeeTypesUpdateForm dbgs = new employeeTypesUpdateForm();
	}
}
